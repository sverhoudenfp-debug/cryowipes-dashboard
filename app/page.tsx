...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar::after {
    content: ''; position: absolute; top: 20%; right: 0; bottom: 20%; width: 1px;
    background: linear-gradient(to bottom, transparent, var(--cyan-glow), transparent); pointer-events: none;
  }
  .nav-btn {
    display: flex; align-items: center; gap: 10px; width: 100%; padding: 10px 12px;
    border-radius: var(--radius-sm); border: none; cursor: pointer; background: transparent;
    color: var(--text-muted); font-weight: 500; font-size: 13px; font-family: var(--font);
    text-align: left; transition: all 0.18s ease; border-left: 2px solid transparent;
    margin-bottom: 2px; position: relative; overflow: hidden;
  }
  .nav-btn::before { content: ''; position: absolute; inset: 0; background: var(--gradient-glow); opacity: 0; transition: opacity 0.18s; }
  .nav-btn:hover { color: var(--text-dim); border-left-color: var(--border-bright); }
  .nav-btn:hover::before { opacity: 1; }
  .nav-btn.active { background: var(--gradient-glow); color: var(--cyan); border-left: 2px solid var(--cyan); font-weight: 600; box-shadow: inset 0 0 20px var(--cyan-dim); }
  .nav-btn.active::before { opacity: 0; }
  .kpi-card {
    background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius);
    padding: 20px; min-width: 0; transition: border-color 0.25s, transform 0.25s, box-shadow 0.25s;
    position: relative; overflow: hidden; cursor: default;
  }
  .kpi-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, var(--cyan-glow), transparent); }
  .kpi-card::after { content: ''; position: absolute; bottom: 0; right: 0; width: 80px; height: 80px; border-radius: 50%; background: var(--cyan-dim); filter: blur(30px); opacity: 0; transition: opacity 0.3s; }
  ...
  .panel { background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); padding: 22px; position: relative; overflow: hidden; }
  ...
  .msg-bubble { padding: 11px 14px; border-radius: 12px; font-size: 12.5px; line-height: 1.7; max-width: 90%; animation: fadeUp 0.2s ease; }
  .msg-bubble.assistant { background: var(--bg-panel); border: 1px solid var(--border); align-self: flex-start; color: var(--text-dim); border-radius: 4px 12px 12px 12px; }
  .msg-bubble.user { background: var(--gradient); color: #fff; align-self: flex-end; border-radius: 12px 12px 4px 12px; font-weight: 500; box-shadow: 0 4px 16px #4f8ef730; }
  .typing-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--cyan); display: inline-block; margin: 0 2px; animation: typingPulse 1.2s ease-in-out infinite; }
  .typing-dot:nth-child(2) { animation-delay: 0.2s; }
  .typing-dot:nth-child(3) { animation-delay: 0.4s; }
  .chat-input-row { display: flex; gap: 7px; margin-top: 10px; flex-shrink: 0; }
  .chat-input { flex: 1; padding: 10px 14px; background: var(--bg-panel); color: var(--text); border: 1px solid var(--border); border-radius: var(--radius-sm); font-size: 12.5px; font-family: var(--font); outline: none; transition: border-color 0.2s, box-shadow 0.2s; }
  .chat-input:focus { border-color: var(--cyan); box-shadow: 0 0 0 3px var(--cyan-dim); }
  .chat-input::placeholder { color: var(--text-muted); }
  .chat-send { padding: 10px 16px; background: var(--gradient); color: #fff; border: none; border-radius: var(--radius-sm); cursor: pointer; font-weight: 600; font-size: 12px; font-family: var(--font); transition: opacity 0.2s, transform 0.1s; white-space: nowrap; box-shadow: 0 4px 16px #4f8ef730; }
  .chat-send:hover { opacity: 0.9; }
  .chat-send:active { transform: scale(0.96); }
  .chat-send:disabled { opacity: 0.35; cursor: not-allowed; }
  .ai-side-panel { width: 0; min-width: 0; background: var(--bg-panel); border-left: 0px solid var(--border); display: flex; flex-direction: column; overflow: hidden; flex-shrink: 0; transition: width 0.35s cubic-bezier(0.4,0,0.2,1), min-width 0.35s cubic-bezier(0.4,0,0.2,1), border-left-width 0.35s; }
  .ai-side-panel.open { width: 360px; min-width: 360px; border-left-width: 1px; }
  .ai-panel-header { padding: 20px 20px 16px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; background: linear-gradient(180deg, var(--bg-card), var(--bg-panel)); }
  .ai-panel-body { flex: 1; display: flex; flex-direction: column; padding: 16px; overflow: hidden; min-height: 0; }
  .ai-fab { position: fixed; bottom: 28px; right: 28px; width: 52px; height: 52px; background: var(--gradient); border: none; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 20px; box-shadow: 0 8px 32px #4f8ef750; transition: transform 0.2s, box-shadow 0.2s; z-index: 48; animation: fabPulse 3s ease-in-out infinite; }
  .ai-fab:hover { transform: scale(1.1); box-shadow: 0 12px 40px #4f8ef780; }
  .ai-fab.open { animation: none; }
  .action-card { background: var(--bg-panel); border: 1px solid var(--amber-dim); border-radius: var(--radius); padding: 16px 18px; margin-bottom: 10px; animation: fadeUp 0.3s ease; position: relative; overflow: hidden; }
  .action-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, var(--amber-dim), transparent); }
  .badge { display: inline-flex; align-items: center; justify-content: center; padding: 2px 8px; border-radius: var(--radius-xs); font-size: 10px; font-weight: 700; letter-spacing: 0.04em; }
  ...
  .pill-btn { padding: 7px 14px; background: var(--bg-panel); border: 1px solid var(--border); border-radius: var(--radius-sm); color: var(--text-muted); font-size: 12px; cursor: pointer; font-family: var(--font); transition: all 0.15s; font-weight: 500; }
  .pill-btn:hover { border-color: var(--border-bright); color: var(--text-dim); background: var(--bg-hover); }
  .notif-btn { background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px 11px; cursor: pointer; color: var(--text-dim); font-size: 15px; position: relative; transition: border-color 0.2s, background 0.2s; line-height: 1; }
  ...
  .notif-dropdown { position: absolute; top: 48px; right: 0; width: 340px; background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); padding: 16px; z-index: 100; box-shadow: 0 16px 48px #00000080; animation: fadeUp 0.2s ease; }
  .upload-zone { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 32px; border: 2px dashed var(--border); border-radius: var(--radius); cursor: pointer; gap: 8px; transition: border-color 0.2s, background 0.2s; }
  ...
  .date-filter-btn:hover { border-color: var(--cyan); color: var(--cyan); background: var(--cyan-dim); }
  .date-filter-btn.active { border-color: var(--cyan); color: var(--cyan); background: var(--cyan-dim); }
  .date-dropdown {
    position: absolute; top: calc(100% + 8px); right: 0;
    background: var(--bg-card); border: 1px solid var(--border);
    border-radius: var(--radius); overflow: hidden;
    box-shadow: 0 16px 48px #00000080; z-index: 200;
    animation: fadeUp 0.15s ease; min-width: 160px;
  }
  .date-option {
    ...
    transition: background 0.1s, color 0.1s; font-weight: 500;
  }
  .date-option:hover { background: var(--bg-hover); color: var(--text); }
  .date-option.selected { color: var(--cyan); background: var(--cyan-dim); }
  .date-option + .date-option { border-top: 1px solid var(--border); }

  .stat-row { display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; background: var(--bg); border-radius: var(--radius-sm); border: 1px solid var(--border); transition: border-color 0.15s; }
  .stat-row:hover { border-color: var(--border-bright); }
  .chip { padding: 6px 12px; background: var(--bg); border: 1px solid var(--border); border-radius: 20px; color: var(--text-muted); font-size: 11.5px; cursor: pointer; transition: all 0.15s; font-family: var(--font); white-space: nowrap; }
  .chip:hover { border-color: var(--cyan); color: var(--cyan); background: var(--cyan-dim); }

  @keyframes fadeUp { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
  @keyframes typingPulse { 0%, 60%, 100% { transform: scale(1); opacity: 0.3; } 30% { transform: scale(1.5); opacity: 1; } }
  @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }
  @keyframes fabPulse { 0%, 100% { box-shadow: 0 8px 32px #4f8ef750, 0 0 0 0 var(--cyan-glow); } 50% { box-shadow: 0 8px 32px #4f8ef750, 0 0 0 8px transparent; } }
...
const DATE_OPTIONS = [
  { value: 'today', label: 'Vandaag', icon: '☀️' },
    ...
    <div className="action-card">
      ...
      {action.status === 'pending' && (
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onApprove} style={{ padding: '7px 14px', background: 'var(--green-dim)', color: 'var(--green)', border: '1px solid #00e5a025', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontSize: 11.5, fontWeight: 700, fontFamily: 'var(--font)' }}>✓ Goedkeuren</button>
          <button onClick={onReject} style={{ padding: '7px 14px', background: 'var(--red-dim)', color: 'var(--red)', border: '1px solid #ff5c5c25', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontSize: 11.5, fontWeight: 700, fontFamily: 'var(--font)' }}>✕ Afwijzen</button>
        </div>
      )}
      ...
      {seoData && !loading && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {seoData.issues.length === 0 && <div className="panel" style={{ borderColor: 'var(--green)30', color: 'var(--green)', textAlign: 'center', fontSize: 13 }}>✓ Geen problemen gevonden.</div>}
...
export default function Dashboard() {
  ...
  const [messages, setMessages] = useState([{ role: 'assistant', content: 'Hoi! Ik ben je CryoWipes AI manager. Ik kan je data analyseren, inzichten geven en acties voorstellen die jij kan goedkeuren.' }]);
  ...
  // Build chart data from ordersByDay
      ...
      <div style={{ display: 'flex', height: '100vh', background: 'var(--bg)', overflow: 'hidden' }}>
        ...
        <div className="sidebar">
          ...
          <div style={{ padding: '12px 10px', borderTop: '1px solid var(--border)' }}>
            <button onClick={() => setAiOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 12px', background: 'var(--gradient-glow)', border: '1px solid var(--cyan-dim)', borderRadius: 'var(--radius-sm)', cursor: 'pointer', color: 'var(--cyan)', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font)', transition: 'all 0.2s', position: 'relative' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-panel)', flexShrink: 0 }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.02em' }}>{NAV.find(n => n.id === page)?.label || 'Dashboard'}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div ref={dateRef} style={{ position: 'relative' }}>
                <button className={`date-filter-btn ${showDateMenu ? 'active' : ''}`} onClick={() => setShowDateMenu(!showDateMenu)}>
                  <span>📅</span>
                  <span>{currentPeriodLabel}</span>
                  <span style={{ fontSize: 10, opacity: 0.6, marginLeft: 2 }}>{showDateMenu ? '▲' : '▼'}</span>
                </button>
                {showDateMenu && (
              ...
              <div ref={notifRef} style={{ position: 'relative' }}>
                <button className={`notif-btn ${alertCount > 0 ? 'has-alerts' : ''}`} onClick={() => setShowNotifications(!showNotifications)}>
                  🔔
                  {alertCount > 0 && <span style={{ position: 'absolute', top: -5, right: -5, background: 'var(--red)', color: '#fff', borderRadius: '50%', width: 17, height: 17, fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, border: '2px solid var(--bg-panel)' }}>{alertCount}</span>}
                </button>
                {showNotifications && (
            ...
            {page === 'dashboard' && (
              <div className="page-anim">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
                  ...
                  <KpiCard label="Orders" value={String(orders)} sub={currentPeriodLabel} color="var(--green)" icon="📦" />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 16, marginBottom: 16 }}>
                  <div className="panel">
                    <div className="panel-title">📈 Omzet — {currentPeriodLabel}</div>
                    <ResponsiveContainer width="100%" height={160}>
                      <AreaChart data={chartData}>
                        <defs><linearGradient id="grad1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#00d4ff" stopOpacity={0.3} /><stop offset="100%" stopColor="#00d4ff" stopOpacity={0} /></linearGradient></defs>
                  ...
                  <div className="panel">
                    <div className="panel-title">Meta Ads — {currentPeriodLabel}</div>
            ...
            {page === 'shopify' && (
              <div className="page-anim">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 12, marginBottom: 18 }}>
                  <KpiCard label="Omzet" value={`$${revenue}`} sub={`${orders} orders`} color="var(--green)" />
                  <KpiCard label="Orders" value={String(orders)} sub={currentPeriodLabel} color="var(--green)" />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 16 }}>
                  <div className="panel">
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                      <AreaChart data={chartData}>
                        <defs><linearGradient id="grad2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#00e5a0" stopOpacity={0.25} /><stop offset="100%" stopColor="#00e5a0" stopOpacity={0} /></linearGradient></defs>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                        <XAxis dataKey="dag" stroke="var(--border)" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} />
                        <YAxis stroke="var(--border)" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} />
                        <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 12, color: 'var(--text)' }} />
            ...
            {page === 'meta' && (
              <div className="page-anim">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 12, marginBottom: 18 }}>
                  ...
                  <KpiCard label="Impressies" value={Number(metaImpressions).toLocaleString()} sub={currentPeriodLabel} color="var(--purple)" icon="👁" />
                  <KpiCard label="Clicks" value={metaClicks} sub={currentPeriodLabel} color="var(--cyan)" icon="🖱" />
                  ...
                  <KpiCard label="All-time spend" value={`$${metaAmountSpent}`} sub="Totaal ooit" color="var(--amber)" icon="📈" />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 16 }}>
                  <div className="panel">
                    <div className="panel-title">Prestaties — {currentPeriodLabel}</div>
                  ...
                  <div className="panel">
                    ...
                    <a href="https://adsmanager.facebook.com" target="_blank" rel="noreferrer"
                      style={{ display: 'block', padding: '9px 12px', background: 'var(--gradient-glow)', border: '1px solid var(--cyan-dim)', borderRadius: 'var(--radius-sm)', color: 'var(--cyan)', fontSize: 12, textDecoration: 'none', textAlign: 'center', fontWeight: 600 }}>
                ...
                <div className="panel">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <button onClick={() => { const prompt = uploadedImageUrl ? `Maak een Meta campagne aan voor CryoWipes met deze afbeelding URL: ${uploadedImageUrl}. Landingspagina: https://cryowipes.store/products/cryo-wipe-box. Budget: $20/dag. Targeting: USA en Canada, 18-45 jaar.` : 'Maak een Meta campagne aan voor CryoWipes. Landingspagina: https://cryowipes.store/products/cryo-wipe-box. Budget: $20/dag. Targeting: USA en Canada, 18-45 jaar.'; setInput(prompt); setAiOpen(true); }}
                      style={{ padding: '12px', background: 'var(--gradient)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontWeight: 700, fontSize: 13, fontFamily: 'var(--font)', boxShadow: '0 4px 16px #4f8ef730' }}>
          ...
          <div style={{ width: 360, display: 'flex', flexDirection: 'column', height: '100%', minWidth: 0 }}>
            <div className="ai-panel-header">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={() => setAiOpen(false)} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xs)', padding: '6px 10px', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 14, lineHeight: 1, fontFamily: 'var(--font)' }}
